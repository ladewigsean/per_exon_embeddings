...
class MultiClassDataset(Dataset):
    def __init__(self, embeddings_path: str, csv_path: str, label_encoder=None):
        ...
        # Filter to only IDs that exist in the HDF5 file
    ...
    def _compute_max_length(self):
        ...
        for key in self.h5f.keys():
            ...
            if len(shape) == 1:
                # 1-D embeddings (single vector per sample)
                self.max_length = 1
                break
            else:
...
class NominalClassifier(nn.Module):
    ...
    def forward(self, x, lengths):
        # Takes last position embedding: [batch, seq_len, embed] -> [batch, embed]
        x = x[:, -1, :]
...
def split_dataset_into_subsets(dataset):
    df = dataset.get_data()
    train_indices = np.where(df["test_split"] == 0)[0]
    val_indices = np.where(df["test_split"] == 1)[0]
    test_indices = np.where(df["test_split"] == 2)[0]
    train_dataset = MultiClassSubset(dataset, train_indices)
    val_dataset = MultiClassSubset(dataset, val_indices)
    test_dataset = MultiClassSubset(dataset, test_indices)
    return train_dataset, val_dataset, test_dataset, dataset.get_max_length()
