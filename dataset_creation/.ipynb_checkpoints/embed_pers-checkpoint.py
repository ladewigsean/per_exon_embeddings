...
def generate_splits_for_embed(per_res_embed, splits_list ):
    ...
    for split in splits_list:
        exon_embed = per_res_embed[math.floor(last_split/3):math.ceil(split/3), : ].mean(axis=1)
        last_split = split
        new_embed = np.append(new_embed,exon_embed,axis=1)
    new_embed  = np.append(new_embed,per_res_embed[math.floor(last_split/3):].mean(axis=1),axis=1)
    return new_embed 
def generate_same_size_splits(per_res_embed, size=150 ):
    number_nuc = per_res_embed.shape[0]*3
...
def create_embedding(checkpoint, df, emb_types=["per_prot"], output_files=["protein_embeddings.h5"]):
    ...
    with ExitStack() as stack:
        files = []
        hdf_dict = {emb_type:stack.enter_context(h5py.File(output_file, "a")) for output_file,emb_type in zip(output_files,emb_types)}
        for _, row in tqdm(df.iterrows(), total=len(df)):
            ...
            if "per_res" in hdf_dict and header not in hdf_dict["per_res"]:
                hdf_dict["per_res"].create_dataset(name=header, data=embedding_per_res)
...
embed("C:\\Users\\ladew\\Documents\\School_related\\Bachelor_related\\dataset_creation\\CYP_PA_Attempt4.fasta","C:\\Users\\ladew\\Documents\\School_related\\Bachelor_related\\dataset_creation\\CYP_PA_Attempt4.csv",embedding_types=["per_res","per_prot","per_exon","fixed_length_chunks","fixed_total_chunks"])
